...
import boardsSlice, { deleteTask, setTaskStatus } from "../redux/boardsSlice";
import AddEditTaskModal from "./AddEditTaskModal";
import DeleteModal from "./DeleteModal";
import { RootState } from "../redux/store";
import { Board, Column, ITask, Subtask } from "../types";

function TaskModal({ taskIndex, colIndex, setIsTaskModalOpen }: any) {
  ...
  const setOpenDeleteModal = () => setIsDeleteModalOpen(!isDeleteModalOpen);
    ...
    >
      <div className=" scrollbar-hide overflow-y-scroll max-h-[95vh]  my-auto  bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md mx-auto  w-full px-8  py-8 rounded-xl">
        <div className=" relative flex   justify-between w-full items-center">
          ...
          />
          {isElipsisMenuOpen && (
            <ElipsisMenu setOpenDeleteModal={setOpenDeleteModal} type="Task" />
          )}
        </div>
        ...
        <div className="mt-8 flex flex-col space-y-3">
          ...
          <select
            ...
            onChange={onChange}
          >
            {columns.map((col: Column, index: Number | any) => (
